"""
From Pytorch-Human-Pose-Estimation
...
class StackedHourGlass(nn.Module):
    ...
    def __init__(self, in_channels, nChannels, nStack, nModules, numReductions, nJoints):
        ...
        # # # Reduction dimensionality layer
        # self.start_0 = nn.Conv2d(in_channels, 64, kernel_size=7, stride=2, padding=3)
        # self.start_1 = M.BnReluConv(64, self.nChannels, kernelSize=3, stride=1, padding=1)

        # Back to standard model
        self.start = M.BnReluConv(in_channels, 64, kernelSize=7, stride=2, padding=3)
        ...
        for _ in range(self.nStack):
            ...
            _chantojoints.append(nn.Conv2d(self.nChannels, self.nJoints, 1, bias=True))
